"""Tests for FastAPI routes in the JupyterHub Usage Quota Service"""
...
class TestHomeRoute:
    ...
    def test_home_displays_usage_when_authenticated(
        ...
        assert "50.0%" in response.text  # Percentage from mock
    ...
    def test_home_with_high_usage_displays_warning(
        ...
        assert "#ef4444" in response.text  # Red color for high usage
...
class TestOAuthCallbackRoute:
    ...
    def test_callback_with_valid_state_and_code(self, client, app, mock_env_vars, mock_oauth_state):
        ...
        # Verify user stored and state cleared
    ...
    def test_callback_clears_oauth_state_from_session(self, client, app, mock_env_vars, mock_oauth_state):
        ...
        with respx.mock:
            ...
            set_session(client, app, {"oauth_state": mock_oauth_state})
            session = get_session(client, app)
            assert "oauth_state" in session  # Verify it exists
        ...
        session = get_session(client, app)
        assert "oauth_state" not in session  # Verify it's cleared
...
class TestServicePrefixConfiguration:
    ...
    def test_home_route_uses_service_prefix(self, client, mock_env_vars):
        """Home route should be accessible at SERVICE_PREFIX"""
        response = client.get("/services/usage/", follow_redirects=False)
        # Should either redirect (not authenticated) or return 200 (authenticated)
    ...
    def test_oauth_redirect_includes_correct_redirect_uri(self, client, mock_env_vars):
        """OAuth redirect should include SERVICE_PREFIX in redirect_uri"""
        response = client.get("/services/usage/", follow_redirects=False)

        assert response.status_code == 307
        location = response.headers["Location"]

        # Parse the redirect URI from query params
